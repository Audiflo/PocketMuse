...
static bool pwm_timer_cb(void* arg) {
    int16_t sample;
    if (g_rb && g_rb->read(sample)) {
        uint32_t duty = (uint32_t)(sample + 32768) >> 8;
        ledc_set_duty(LEDC_LOW_SPEED_MODE, PWMAudioOutput::kLEDCChannel, duty);
        ledc_update_duty(LEDC_LOW_SPEED_MODE, PWMAudioOutput::kLEDCChannel);
    }
    return false;
}

PWMAudioOutput::PWMAudioOutput(RingBuffer& rb, int pin)
    : rb_(rb), pin_(pin), running_(false) {}
...
bool PWMAudioOutput::begin() {
    ...
    timer_set_alarm_value(TIMER_GROUP_0, TIMER_0, 40000000 / kSampleRate);
